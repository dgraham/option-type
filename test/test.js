...
describe('Result<T, E>', function () {
  ...
  describe('andThen', function () {
    it('flat maps ok and err', function () {
      ...
      assert.equal(Ok(2).andThen(sq).andThen(sq).unwrap(), 16);
      assert(Ok(2).andThen(sq).andThen(nope).isErr());
      assert(Ok(2).andThen(nope).andThen(sq).isErr());
      assert(Err('boom').andThen(sq).andThen(sq).isErr());
    });
  });

  describe('or', function () {
    it('returns the ok over err', function () {
      assert.equal(Ok(2).or(Err('boom')).unwrap(), 2);
    });

    it('returns the alternative to err', function () {
      assert.equal(Err('boom').or(Ok(2)).unwrap(), 2);
    });

    it('returns the first of two ok values', function () {
      assert.equal(Ok(2).or(Ok(100)).unwrap(), 2);
    });

    it('returns err for two err values', function () {
      assert(Err('one').or(Err('two')).isErr());
      assert.equal(Err('one').or(Err('two')).unwrapErr(), 'two');
    });
  });
  ...
  describe('orElse', function () {
    ...
    it('returns the ok', function () {
      assert.equal(Ok(2).orElse(def).unwrap(), 2);
    });

    it('returns the default ok value', function () {
      assert.equal(Err('boom').orElse(def).unwrap(), 42);
    });

    it('returns the default err value', function () {
      assert(Err('one').orElse(nope).isErr());
      assert.equal(Err('one').orElse(nope).unwrapErr(), 'boom');
    });
  });
